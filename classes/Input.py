...
class Input:
    ...
    def checkForQuitAndRestartInputEvents(self, events):
        for event in events:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_f and (event.mod & pygame.KMOD_SHIFT):
                    if not window_state.fullscreen:
                        window_state.screen = window_state.set_screen(window_state.windowSize, True)
                        self._update_screen_refs()
                # Exit fullscreen: Shift+W
                elif event.key == pygame.K_w and (event.mod & pygame.KMOD_SHIFT):
                    print('Shift+W detected')  # Debug
                    if window_state.fullscreen:
                        window_state.screen = window_state.set_screen(window_state.windowSize, False)
                        self._update_screen_refs()
                # Pause/game menu (ESC)
    ...
    def _update_screen_refs(self):
        self.entity.screen = window_state.screen
        if hasattr(self.entity, 'pauseObj'):
            self.entity.pauseObj.screen = window_state.screen
        if hasattr(self.entity, 'dashboard'):
            self.entity.dashboard.screen = window_state.screen
        if hasattr(self.entity, 'levelObj'):
            self.entity.levelObj.screen = window_state.screen
